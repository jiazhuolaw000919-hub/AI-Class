...
class RuntimePermissionSystem {
    ...
    init() {
        ...
        this._emitEvent('permissionSystem.initialized', { version: this.version, permissions: this.systemState.totalPermissions, subjects: this.systemState.totalSubjects, resources: this.systemState.totalResources });
    }
    ...
    registerSubject(def) {
        ...
        if (validTypes.indexOf(type) === -1) throw new Error('Invalid subject type: ' + type);
        ...
        var subject = {
            ...
            name: name,
            status: 'active',
            metadata: Object.assign({ registeredAt: new Date().toISOString(), registeredBy: 'system' }, metadata),
            stats: { accessAttempts: 0, granted: 0, denied: 0, lastAccess: null }
        };
    ...
    getSubject(subjectId) { return this.subjectRegistry.get(subjectId) || null; }
    ...
    getResource(resourceId) { return this.resourceRegistry.get(resourceId) || null; }
    ...
    grantPermission(def) {
        ...
        if (!this.subjectRegistry.has(subjectId)) throw new Error('Subject ' + subjectId + ' not found');
        if (resourceId !== '*' && !this.resourceRegistry.has(resourceId)) throw new Error('Resource ' + resourceId + ' not found');
        var validActions = Object.keys(this.accessActions);
        if (validActions.indexOf(action) === -1) throw new Error('Invalid action: ' + action);
        ...
        var permission = {
            ...
            stats: { used: 0, lastUsed: null }
        };
        
        this.permissions.set(permission.permissionId, permission);
        this.systemState.totalPermissions = this.permissions.size;
        this.systemState.activePermissions = this._countActivePermissions();
        this._audit('PERMISSION_GRANTED', { permissionId: permission.permissionId, subjectId: subjectId, resourceId: resourceId, action: action, scope: scope });
        return permission;
    }
    ...
    revokePermission(permissionId, reason) {
        ...
        this.systemState.activePermissions = this._countActivePermissions();
        this._audit('PERMISSION_REVOKED', { permissionId: permissionId, oldStatus: oldStatus, reason: reason });
        return permission;
    }
    ...
    checkAccess(subjectId, resourceId, action, context) {
        ...
        if (relevantPermissions.length === 0) {
            var result3 = this._createAccessResult(accessId, subjectId, resourceId, action, false, 'No permission granted (Least Privilege principle)');
        ...
        for (var i = 0; i < relevantPermissions.length; i++) {
            ...
            if (this._isSensitiveAction(action)) {
                ...
                if (!sensitiveCheck.passed) {
                    var result4 = this._createAccessResult(accessId, subjectId, resourceId, action, false, 'Sensitive action verification failed: ' + sensitiveCheck.reason, { sensitiveCheck: sensitiveCheck });
            ...
            var result5 = this._createAccessResult(accessId, subjectId, resourceId, action, true, 'Access granted via permission ' + permission.permissionId + ' (' + permission.action + ')', { permissionId: permission.permissionId, actionLevel: permission.actionLevel });
            result5.evaluationTime = (performance.now() - startTime).toFixed(2) + 'ms';
            
            this._recordAccessAttempt(result5);
            subject.stats.granted++;
            this._audit('ACCESS_GRANTED', { accessId: accessId, subjectId: subjectId, resourceId: resourceId, action: action, permissionId: permission.permissionId, evaluationTime: result5.evaluationTime });
            return result5;
        }
        
        var result6 = this._createAccessResult(accessId, subjectId, resourceId, action, false, 'No valid permission found');
        this._recordAccessAttempt(result6);
        subject.stats.denied++;
        return result6;
    }
    
    canRead(subjectId, resourceId, context) { return this.checkAccess(subjectId, resourceId, 'READ', context); }
    canModify(subjectId, resourceId, context) { return this.checkAccess(subjectId, resourceId, 'MODIFY', context); }
    canExecute(subjectId, resourceId, context) { return this.checkAccess(subjectId, resourceId, 'EXECUTE', context); }
    ...
    getEffectivePermissions(subjectId, minimumAction) {
        ...
        var all = Array.from(this.permissions.values());
        for (var i = 0; i < all.length; i++) {
            if (all[i].subjectId === subjectId && all[i].status === this.statuses.ACTIVE && all[i].actionLevel >= minLevel) {
                result.push(all[i]);
            }
        }
        return result;
    ...
    getReport() {
        ...
        for (var i = 0; i < actions.length; i++) {
            ...
            for (var j = 0; j < all.length; j++) {
                if (all[j].action === actions[i] && all[j].status === this.statuses.ACTIVE) count++;
            }
            permissionsByAction[actions[i]] = count;
        }
        
        var permissionsBySubject = {};
        var subjects = Array.from(this.subjectRegistry.entries());
        ...
        return {
            ...
            subjects: { total: this.systemState.totalSubjects },
            resources: { total: this.systemState.totalResources },
            permissions: { total: this.systemState.totalPermissions, active: this.systemState.activePermissions, byAction: permissionsByAction, bySubject: permissionsBySubject },
            access: {
                ...
                grantRate: this.systemState.accessAttempts.total > 0 ? ((this.systemState.accessAttempts.granted / this.systemState.accessAttempts.total) * 100).toFixed(1) + '%' : 'N/A'
            },
            securityRules: ['Rule 1: Default Least Privilege ✅', 'Rule 2: Sensitive action verification ✅', 'Rule 3: Permission change audit ✅', "Rule 4: Permission failure doesn't affect runtime ✅"],
            recentAudit: this.auditLog.slice(-10)
        };
    }
    
    getHealth() {
        var denyRate = this.systemState.accessAttempts.total > 0 ? this.systemState.accessAttempts.denied / this.systemState.accessAttempts.total : 0;
        var health = 'HEALTHY';
        ...
        return {
            status: health, version: this.version,
            ...
            accessGrantRate: this.systemState.accessAttempts.total > 0 ? ((this.systemState.accessAttempts.granted / this.systemState.accessAttempts.total) * 100).toFixed(1) + '%' : 'N/A',
            lastAccess: this.systemState.lastAccess,
            isOperational: true
        };
    }
    
    getAuditTrail(limit) { return this.auditLog.slice(-(limit || 100)); }
    getAccessHistory(limit) { return this.accessAttempts.slice(-(limit || 50)); }
    
    // ========== PRIVATE ==========
    
    _registerDefaultSubjects() {
        this.registerSubject({ subjectId: 'SUB-DEV-001', type: this.subjectTypes.DEVELOPER, name: 'System Developer', metadata: { description: 'Full development access' } });
        this.registerSubject({ subjectId: 'SUB-AI-001', type: this.subjectTypes.AI_ASSISTANT, name: 'AI Runtime Assistant', metadata: { description: 'Analysis and recommendation access' } });
        this.registerSubject({ subjectId: 'SUB-MOD-BOOT', type: this.subjectTypes.RUNTIME_MODULE, name: 'BootManager Module', metadata: { description: 'Boot management module', module: 'BootManager' } });
        this.registerSubject({ subjectId: 'SUB-MOD-STATE', type: this.subjectTypes.RUNTIME_MODULE, name: 'StateSyncEngine Module', metadata: { description: 'State synchronization module', module: 'StateSyncEngine' } });
        this.registerSubject({ subjectId: 'SUB-AUTO-001', type: this.subjectTypes.AUTOMATION_AGENT, name: 'Automation Agent', metadata: { description: 'Automated task execution agent' } });
        this.registerSubject({ subjectId: 'SUB-SYS-001', type: this.subjectTypes.SYSTEM, name: 'System Core', metadata: { description: 'Core system process' } });
    }
    
    _registerDefaultResources() {
        this.registerResource({ resourceId: 'RES-RUNTIME', name: 'Runtime Core', type: 'system', sensitivity: 'critical' });
        this.registerResource({ resourceId: 'RES-BOOT', name: 'Boot Manager', type: 'system', sensitivity: 'critical' });
        this.registerResource({ resourceId: 'RES-STATE', name: 'State Store', type: 'data', sensitivity: 'sensitive' });
        this.registerResource({ resourceId: 'RES-METRICS', name: 'Runtime Metrics', type: 'data', sensitivity: 'normal' });
        this.registerResource({ resourceId: 'RES-EVENTS', name: 'Event Store', type: 'data', sensitivity: 'normal' });
        this.registerResource({ resourceId: 'RES-AI', name: 'AI Engine', type: 'intelligence', sensitivity: 'sensitive' });
        this.registerResource({ resourceId: 'RES-KNOWLEDGE', name: 'Knowledge Graph', type: 'intelligence', sensitivity: 'normal' });
        this.registerResource({ resourceId: 'RES-CONFIG', name: 'System Configuration', type: 'system', sensitivity: 'critical' });
        this.registerResource({ resourceId: 'RES-LOGS', name: 'System Logs', type: 'data', sensitivity: 'normal' });
    }
    
    _grantDefaultPermissions() {
        this.grantPermission({ subjectId: 'SUB-DEV-001', resourceId: '*', action: 'EXECUTE', scope: ['*'], metadata: { reason: 'Developer requires full system access' } });
        this.grantPermission({ subjectId: 'SUB-AI-001', resourceId: '*', action: 'RECOMMEND', scope: ['*'], metadata: { reason: 'AI can analyze and recommend, but not modify' } });
        this.grantPermission({ subjectId: 'SUB-AI-001', resourceId: 'RES-METRICS', action: 'ANALYZE', scope: ['*'], metadata: { reason: 'AI needs metrics for analysis' } });
        this.grantPermission({ subjectId: 'SUB-AI-001', resourceId: 'RES-EVENTS', action: 'ANALYZE', scope: ['*'], metadata: { reason: 'AI needs events for analysis' } });
        this.grantPermission({ subjectId: 'SUB-AI-001', resourceId: 'RES-KNOWLEDGE', action: 'ANALYZE', scope: ['*'], metadata: { reason: 'AI needs knowledge graph for context' } });
        this.grantPermission({ subjectId: 'SUB-MOD-BOOT', resourceId: 'RES-BOOT', action: 'EXECUTE', scope: ['boot'], metadata: { reason: 'BootManager manages boot process' } });
        this.grantPermission({ subjectId: 'SUB-MOD-BOOT', resourceId: 'RES-RUNTIME', action: 'MODIFY', scope: ['boot'], metadata: { reason: 'BootManager can modify runtime during boot' } });
        this.grantPermission({ subjectId: 'SUB-MOD-STATE', resourceId: 'RES-STATE', action: 'MODIFY', scope: ['state'], metadata: { reason: 'StateSyncEngine manages state' } });
        this.grantPermission({ subjectId: 'SUB-AUTO-001', resourceId: 'RES-METRICS', action: 'READ', scope: ['*'], metadata: { reason: 'Automation agent can read metrics' } });
        this.grantPermission({ subjectId: 'SUB-AUTO-001', resourceId: 'RES-LOGS', action: 'READ', scope: ['*'], metadata: { reason: 'Automation agent can read logs' } });
        this.grantPermission({ subjectId: 'SUB-SYS-001', resourceId: '*', action: 'EXECUTE', scope: ['*'], metadata: { reason: 'System core requires unrestricted access' } });
    }
    ...
    _isSensitiveAction(action) { return this.sensitiveActions.indexOf(action) !== -1; }
    ...
    _validateSensitiveAction(subject, resource, action, context) {
        if (resource && resource.sensitivity === 'critical') {
            ...
            if (allowedTypes.indexOf(subject.type) === -1) {
                return { passed: false, reason: 'Subject type "' + subject.type + '" cannot perform ' + action + ' on critical resource "' + resource.name + '"' };
            }
        }
        if (!context.reason && !context.approved) {
            return { passed: false, reason: 'Sensitive action "' + action + '" requires a reason or approval in context' };
        }
        if (subject.type === this.subjectTypes.AUTOMATION_AGENT && !context.approved) {
            return { passed: false, reason: 'Automation agents require explicit approval for sensitive actions' };
        }
    ...
    _createAccessResult(accessId, subjectId, resourceId, action, granted, reason, details) {
        return {
            accessId: accessId, subjectId: subjectId, resourceId: resourceId,
    ...
    _recordAccessAttempt(result) {
        ...
        if (result.granted) this.systemState.accessAttempts.granted++;
        else this.systemState.accessAttempts.denied++;
        if (this.accessAttempts.length > 500) this.accessAttempts = this.accessAttempts.slice(-250);
    }
    ...
    _emitEvent(type, data) {
        try {
            ...
            if (typeof emitFn === 'function') {
                emitFn.call(collector, { type: type, source: 'RuntimePermissionSystem', data: data, timestamp: new Date().toISOString() });
            }
        } catch (e) {}
    }
