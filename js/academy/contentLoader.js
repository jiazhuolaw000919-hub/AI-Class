...
// 保留所有原有功能，新增 S4 加载能力
// v2.1.0 — installDefaultAIPack 清空假数据
//
// ⚠️ v2.1.0 变更:
//   旧版 installDefaultAIPack 会往 localStorage 塞入假 pack
//   (course_ai_foundation / course_prompt_engineering 等下划线命名的假数据)，
//   与真实的 /content/courses/course-ai/course.json 冲突，
//   导致 UI 遍历时对假 course 发 fetch → 404。
//
//   现在 installDefaultAIPack 改为：
//   1. 清掉旧的假 pack 缓存 (pack_academy_ai)
//   2. 不再注入任何假数据
//   3. 所有真实 course 由 loadCourse() 从磁盘加载
...
(function() {
    ...
    const S4ContentLoader = {
        ...
        // ============================================================
        // ⚠️ v2.1.0: installDefaultAIPack 已废弃
        // ============================================================
        // 旧版这里硬编码了 course_ai_foundation / course_prompt_engineering
        // 等**下划线命名**的假 course，缓存到 localStorage 的 pack_academy_ai。
        //
        // 这些假 course 在 /content/courses/ 下**根本不存在**，
        // 导致 UI 遍历 getAllCourses() 时对每个假 course 发 fetch → 404。
        //
        // 现在改为：清掉旧的假 pack，不注入任何假数据。
        // 如果确实需要 fallback，请确保磁盘上有对应文件！
        installDefaultAIPack: function() {
            console.log('[S4ContentLoader] ⏭️ installDefaultAIPack: skipped (v2.1.0 deprecated)');

            // 清掉旧的假 pack 缓存
            try {
                if (window.LawAIApp?.StorageEngine && typeof window.LawAIApp.StorageEngine.remove === 'function') {
                    window.LawAIApp.StorageEngine.remove('pack_academy_ai');
                }
                if (typeof localStorage !== 'undefined') {
                    localStorage.removeItem('pack_academy_ai');
                }
                console.log('[S4ContentLoader] 🧹 Cleaned legacy pack_academy_ai cache');
            } catch (e) {
                console.warn('[S4ContentLoader] Clean pack_academy_ai failed:', e);
            }
    ...
    window.LawAIApp.S4ContentLoader = S4ContentLoader;

    console.log('[S4ContentLoader] ✅ Extended with S4 methods (v2.1.0)');
